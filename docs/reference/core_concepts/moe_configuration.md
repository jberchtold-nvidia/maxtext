...
`te_ep_receive_capacity_fraction`: TE MoEBlock receive capacity relative to its aligned dropless worst case. It must be in `(0, 1]`: `1.0` uses full worst-case capacity, while `0.5` uses half. An overflowing step skips its optimizer update.

`te_ep_overflow_check_every_n_steps`: Number of steps between host-side checks of buffered TE EP overflow results. It defaults to 20. An overflowing step still skips its optimizer update immediately on device; MaxText reports the overflow and exits at the next check.
