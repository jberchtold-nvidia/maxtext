...
- With `te_moe_block=true`, values must be at least 1.0 and scale TE's aligned perfectly-balanced receive capacity. MaxText skips the optimizer update and exits with an error if routing exceeds the configured capacity.
