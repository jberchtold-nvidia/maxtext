...
with open(input_csv) as f:
    ...
    for row in reader:
        key = tuple(row.get(k) if row.get(k) not in [None, ''] else 'NA' for k in ['dp', 'tp', 'tpsp', 'fsdp'])
        if not row.get('test'):
            continue
        if key not in data:
            data[key] = {}
...
for key in sorted(data.keys()):
    ...
    for testname, row in rowset.items():
        mean = row['mean']
        stddev = row['stddev']
        if mean == 'NA':
            normalized = "-"
            ...
            normalized = "-"
        rows.append([testname, row['dp'], row['tp'], row['tpsp'], row['fsdp'], mean, stddev, normalized])
